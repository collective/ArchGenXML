...
#
# Created:     2003/16/04
# RCS-ID:      $Id: ArchGenXML.py,v 1.117 2004/04/03 18:17:34 zworkb Exp $
...
'''
    def generateStdFiles(self,target,package,projectName):
        if package.isRoot():
            self.generateStdFilesForProduct(target,package,projectName)
        else:
            self.generateStdFilesForPackage(target,package,projectName)
            
    def generateStdFilesForPackage(self,target,package,projectName):
        if target[-1] in ('/','\\'):
            target=target[:-1]
    ...
    def generateStdFilesForProduct(self, target,package,projectName):
        ...
        if tool_classes:
            toolinit=self.TEMPL_TOOLINIT % ','.join([c.getQualifiedName(package) for c,m in self.generatedClasses if c.hasStereoType(self.portal_tools)])
        else: toolinit=''
...
<configuration>'''
    ...
    def generatePackage(self,package,recursive=1):
        ...
        for element in package.getClasses()+package.getInterfaces():
            ...
            try:
                ...
                if not element.isInterface():
                    ...
                    package.generatedClasses.append([element,module])
                else:
        ...
        self.generateStdFiles(package.getFilePath(),package,self.projectName)

    def generateProduct(self, root, projectName=None ):
        dirMode=0
        outfile=None
...
is the right place."""
        ...
        makeDir(os.path.join(root.getFilePath(),'skins',self.projectName))
        makeDir(os.path.join(root.getFilePath(),'skins',
        ...
        self.generatePackage(root,projectName)
        if self.ape_support:
            self.generateApeConf(root.getFilePath(),projectName)
