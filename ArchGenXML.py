...
#
# Created:     2003/16/04
# RCS-ID:      $Id: ArchGenXML.py,v 1.89 2004/01/29 09:18:15 yenzenz Exp $
...
class ArchetypesGenerator:
    ...
    nonstring_tgvs=['widget','vocabulary'] #taggedValues that are not strings, e.g. widget or vocabulary
    ...
    def generateMethodActions(self,element):
        ...
        for m in element.getMethodDefs():
            if m.getStereoType() in ['action','view','form']:
                ...
                dict={}
                dict['action']=getExpression(action_name)
...
%(other)s
    ...
    def getWidget(self, type, element):
        ...
        if tgv.has_key('widget'):
            ...
            custom=1
            widgetcode = '''widget=%s''' % tgv['widget']
...
"""
    ...
    def generateStdFiles(self, target,projectName,generatedModules):
        ...
        for c in [cn[0] for cn in self.generatedClasses
            ...
            configlet_icon=     c.getTaggedValue('configlet_icon','plone_icon')
...
def main():
    ...
    opts, args = getopt.getopt(args, 'f:a:t:o:s:p:P:n',
                              ['ape','actions','default-actions','no-actions',
                               ...
                               'author=','e-mail=','copyright=','licence='])
    prefix = ''
    outfileName = None
...
Options:
    -o <target>                  Output file directory path for data 
                                 representation classes. Last part of used for 
                                 internal directory namings.
    -P <name1>,<name2>...        names of packages to parse in source file 
    -a --actions                 generates actions (default)
    --no-actions                 do not generates actions
    --method-preservation        methods in the target sources will be preserved
                                 (default)
    --no-method-preservation     methods in the target sources will be preserved
    -t --unknown-types-as-string unknown attribute types will be treated as text
    --ape-support                generate apeconf.xml and generators for ape 
                                 (needs Archetypes 1.1+)
    --i18n-support               support for i18NArchetypes:
                                 attributes with a stereotype 'i18n' or a 
                                 taggedValue 'i18n' set to '1' are multilingual
                                 
    --no-module-info-header      do not generate module info header
    --author=<string>            set default author string for module info
                                 headers, taggedValue will override this
    --e-mail=<string>            set default e-mail adress string for module 
                                 info headers, taggedValue will override this
    --copyright=<string>         set default copyright string for module 
                                 info headers, taggedValue will override this
    --licence=<string>           set default licence string for module 
                                 info-headers, taggedValue will override this
