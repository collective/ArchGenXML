...
#
# Created:     2003/16/04
# RCS-ID:      $Id: ArchGenXML.py,v 1.123 2004/04/10 02:12:14 zworkb Exp $
...
%(other)s
    ...
    def generateMethod(self,outfile,m,klass,mode='class'):
        ...
        if m.hasStereoType('portlet_view'):
            ...
            f=makeFile(os.path.join(klass.getRootPackage().getFilePath(),'skins',klass.getPackage().getRootPackage().getName(),view_name+'.pt'),0)
            if f:
                templdir=os.path.join(sys.path[0],'templates')
